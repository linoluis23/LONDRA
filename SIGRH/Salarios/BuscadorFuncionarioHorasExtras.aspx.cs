...
public partial class Salarios_BuscadorFuncionario_HorasExtras : System.Web.UI.Page
{
    ...
    protected void gv_items_RowCommand(object sender, GridViewCommandEventArgs e)
    {
        ...
        switch (e.CommandName)
        {
            case "GetAssign":
                ...
                //Response.Redirect("RegistroHorasExtras?id=" + ci);
                Response.Redirect("RegistroHorasExtras?id=" + per_id + "&id2=" + as_id);
                break;
    ...
    protected void BtnBuscar_Click(object sender, EventArgs e)
    {
        ...
        else
        {
            ...
            {
                sc = "$.notify({ icon: 'fas fa-exclamation', message: 'No corresponde' }, { type: 'warning', placement: { from: 'bottom', align: 'right'} }); $('#blockResultados').css('display', 'none');";
            }
        }
        Txt_per_num_doc_b.Focus();
        SetScript(sc);
    ...
    //Grilla de Horas Extras
    protected void btn_ver_he_Click(object sender, EventArgs e)
    {
        sc = "MostrarMascara(false); $('#modalHorasExtras').modal('show');";
    ...
    private void listarGrillaHorasExtras()
    {
        try
        {
            ...
            int id_pc = obtenerIdProceso();
            factor.tr_pc_id = id_pc;
            var grillaHoras = factor.ObtenerGrillaHorasExtras();
            int num = grillaHoras.Tables[0].Rows.Count;
            block_horasExtras.Visible = (num > 0) ? false : true;
