...
public partial class MovimientoPersonal_RecuperacionRetencionAlta : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if (HttpContext.Current.Session["per_id"] != null && HttpContext.Current.Session["per_id"].ToString() != "")
        {
            if (!Page.IsPostBack)
            {
                ...
                CargarAsignaciones(Convert.ToInt32(id));
                BindForm(id);
    ...
    private void BindForm(string per_id)
    {
        ...
        var var_datos_as_c = _asignacion.ObtenerTablaGrillaC2("", per_id, ddl_asignacion.SelectedValue, "", "", "", "", "", "", "", "", "", "", "").Tables[0];
        ...
        if (var_datos_as_c.Rows.Count > 0)
        {
            ...
            ltl_jornada.Text = var_datos_as_c.Rows[0]["ca_tipo_jornada_lit"].ToString();
    ...
    private void BindDDLTipoTransaccion()
    {
        ...
        Ddl_tr_fa_id.DataSource = _factor.ListarDescuentosMontosCuotas();// _factor.ObtenerTablaComboX("51, 52, 54, 0");
        Ddl_tr_fa_id.DataValueField = "fa_id";
        Ddl_tr_fa_id.DataTextField = "fa_descripcion";
        Ddl_tr_fa_id.DataBind();

        Txt_tc_cant_cuotas.DataSource = _factor.ListarTotalCuotas();
        Txt_tc_cant_cuotas.DataTextField = "CUOTA";
        Txt_tc_cant_cuotas.DataValueField = "CUOTA";
        Txt_tc_cant_cuotas.DataBind();
    }
    ...
    {
        cls_pla_transacciones transaccion = new cls_pla_transacciones();
        transaccion.QuitarDescuento(Convert.ToInt32(Hf_tr_id_b.Value));
        var id = Request.QueryString["id"].ToString();
        BindGridView(id);
        if (GvLista.Rows.Count == 0)
        {
    ...
    protected void BtnGuardarG_Click(object sender, EventArgs e)
    {
        ...
        //    tr_pc_id = Convert.ToInt32(ddl_asignacion.SelectedValue),
        ...
        factor.tr_pc_id = Convert.ToInt32(ddl_asignacion.SelectedValue);
        factor.tr_per_id = Convert.ToInt32(id);
        factor.tr_fa_id = Convert.ToInt32(Ddl_tr_fa_id.SelectedValue);
        factor.tr_monto = Txt_tr_monto.Text.Trim();
        factor.tr_usuario_creacion = Convert.ToInt32(HttpContext.Current.Session["per_id"].ToString());
        var tr_id= factor.AdicionarTransaccionMontoUnico();

        if (P_cuotas.Visible)
        {
            var montt = Convert.ToDouble(Txt_tr_monto.Text.Trim());
            var cantc = Convert.ToInt32(Txt_tc_cant_cuotas.SelectedItem.Text.Trim());
            ...
            for (int i = 0; i < cantc; i++)
            {
                ...
                cls_pla_factor cuotas = new cls_pla_factor();
                cuotas.tr_id = tr_id;
                cuotas.tr_monto = montc;
                cuotas.tr_pc_id= Convert.ToInt32(ddl_asignacion.SelectedValue);
                cuotas.tr_per_id = Convert.ToInt32(id);
                cuotas.tr_fa_id= Convert.ToInt32(Ddl_tr_fa_id.SelectedValue);
                cuotas.tr_usuario_creacion= Convert.ToInt32(HttpContext.Current.Session["per_id"].ToString());
                cuotas.AdicionarTransaccionPorCuotas(cuotas);
            }
        }
