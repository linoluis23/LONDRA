...
public partial class Precontratacion_FrecuenciaInicio : System.Web.UI.Page
{
    ...
    protected void lv_categorias_ItemCommand(object sender, ListViewCommandEventArgs e)
    {
        ...
        string estado = lv_categorias.DataKeys[index].Values[2].ToString();
        ...
        switch (e.CommandName)
        {
            case "GetAssign":
                if (estado != "VALIDADO")
                {
                    Response.Redirect("FrecuenciaInicio?id=" + cp_id + "&id2=" + Session["pr_id"].ToString() + "&id3=" + cod_poa);
                }
                else
                {
                    sc = "$.notify({ icon: 'fa fa-info-circle', message: 'No se puede realizar la acción, las frecuencias de la categoría seleccionada ya han sido validadas'},{type: 'info', placement: { from: 'bottom', align: 'right'} });";
                    SetScript(sc);
                }
                break;
            case "GetDetailCat":
