...
public partial class Precontratacion_PlanillaValidada : System.Web.UI.Page
{
    ...
    protected void gv_planilla_RowCommand(object sender, GridViewCommandEventArgs e)
    {
        ...
        hf_pre_estado.Value = gv_planilla.Rows[index].Cells[15].Text;
    ...
    private void datosPuesto()
    {
        ...
        if (detallePuesto.Tables.Count > 0)
        {
            if (detallePuesto.Tables[0].Rows.Count > 0)
            {
                ...
                var puesto = ddl_puesto_modificar.Items.FindByValue(validarCampo(puestoX["pre_p_id"]));
                ddl_puesto_modificar.SelectedValue = (puesto != null) ? puesto.Value : "0";
    ...
    private string datosFuncionario()
    {
        ...
        if (detalle_precontrato.Tables.Count > 0)
        {
            if (detalle_precontrato.Tables[0].Rows.Count > 0)
            {
                ...
                chk_pre_ver_ap_casada.Checked = (validarCampo(funcionarioX["mostrar_materno_x"]) != "") ? Convert.ToBoolean(validarCampo(funcionarioX["mostrar_materno_x"])) : false;
                ...
                string imagen = (funcionarioX["fp_foto"] != DBNull.Value && funcionarioX["fp_foto"].ToString().Trim() != "") ? funcionarioX["fp_foto"].ToString().Trim() : "";
        ...
        if (hf_pre_per_id.Value != "0")
        {
            ...
            js = " $('#ver_casada').css('display', 'none'); ";
        }
        else
        {
            ...
            if (ddl_pre_genero.SelectedValue.Trim() == "F")
            {
                if (chk_pre_ver_ap_casada.Checked)
                {
                    js = " $('#ver_casada').css('display', 'block'); ";
                }
                else
                {
                    js = " $('#ver_casada').css('display', 'none'); ";
    ...
    private void listarFrecuencias(int cp_id = 0, int fr_es_id = 0, string fr_descrip_puesto = "0", decimal fr_tiempo = 0)
    {
        ...
        try
        {
            ...
            listaFiltradoFrec(cp_id, pr_id, fr_es_id, fr_descrip_puesto, fr_tiempo, "C15", "ddl_gv_frec_tiempo", "fr_tiempo", "fr_tiempo", "ROUND((F.fr_tiempo - ISNULL(PC.tiempo, 0)), 4) AS fr_tiempo");
        }
        catch (Exception e)
        {
            Console.Error.Write(e.Message);
    ...
    private void datosfrecuencia()
    {
        ...
        sc = "$('#modalAdicionarFrecPost').modal('show');";
        SetScript(sc);
    }
    ...
    protected void btn_adicionar_frecuencia_Click(object sender, EventArgs e)
    {
        ...
        listarPlanilla(pl_id);
        limpiar();
        sc = "MostrarMascara(false); Swal.fire({ icon: 'success', title: 'Pre-Contrato creado correctamente.', text: 'Registro exitoso', timer: 2200, showConfirmButton: false, allowOutsideClick: false, onAfterClose: () => { $('#modalAdicionarFrec').modal('hide'); $('#modalAdicionarFrecPost').modal('hide'); $('body').removeClass('modal-open'); $('.modal-backdrop').remove(); }});";
        SetScript(sc);
    }
    ...
    {
        limpiar();
        sc = "$('#modalAdicionarFrecPost').modal('hide');";
        SetScript(sc);
    }
    ...
    private void informacionPlanilla(int pl_id = 0)
    {
        ...
        if (detalle_planilla.Tables.Count > 0)
        {
            if (detalle_planilla.Tables[0].Rows.Count > 0)
            {
                ...
                ltl_pl_ue.Text = validarCampo(planillaX["ue"]);
            }
        }
    }
    ...
    protected void txt_ci_TextChanged(object sender, EventArgs e)
    {
        ...
        SetScript("$('#ver_casada').css('display', 'none');");
    }
    ...
    protected void txt_pre_paterno_TextChanged(object sender, EventArgs e)
    {
        ...
        sc = (ddl_pre_genero.SelectedValue.Trim() == "F") ? " $('#ver_casada').css('display', 'block'); " : " $('#ver_casada').css('display', 'none'); ";
        SetScript(sc);
    ...
    protected void ddl_pre_genero_SelectedIndexChanged(object sender, EventArgs e)
    {
        if (ddl_pre_genero.SelectedValue == "F")
        {
            sc = " $('#ver_casada').css('display', 'block'); ";
        }
        else
        {
            chk_pre_ver_ap_casada.Checked = false;
            sc = " $('#ver_casada').css('display', 'none'); ";
        }
        SetScript(sc);
    }
    private void llenarDatosFuncionario(DataRow detalle_funcionario)
    {
        ...
        string imagen = (funcionarioX["Imagen"] != DBNull.Value && funcionarioX["Imagen"].ToString().Trim() != "") ? funcionarioX["Imagen"].ToString().Trim() : "";
        ...
        if (imagen != "")
        {
            imgFun_int.ImageUrl = "data:image/jpg;base64," + Convert.ToBase64String((byte[])funcionarioX["Imagen"]);
        }
        else
        {
            if (sexo_int == "")
    ...
    private bool fechasFrecuenciaCorrecto()
    {
        ...
        if (detalle_contrato.Tables.Count > 0)
        {
            if (detalle_contrato.Tables[0].Rows.Count > 0)
            {
                ...
                if ((startDate >= frecStartDate && startDate <= frecEndDate) && (endDate >= frecStartDate && endDate <= frecEndDate))
                {
                    if (startDate > lastEndDate && endDate <= frecEndDate)
                    {
                        sw = true;
                    }
                }
    ...
    protected void chk_marcar_todos_CheckedChanged(object sender, EventArgs e)
    {
        for (int i = 0; i < gv_planilla.Rows.Count; i++)
        {
            GridViewRow row = gv_planilla.Rows[i];
            bool isChecked = ((CheckBox)gv_planilla.HeaderRow.FindControl("chk_marcar_todos")).Checked;

            if (isChecked)
            {
                ((CheckBox)row.FindControl("chk_marcar")).Checked = true;
            }
            else
            {
                ((CheckBox)row.FindControl("chk_marcar")).Checked = false;
            }
        }
        SetScript("");
    }

    protected void btn_modificar_fecha_Click(object sender, EventArgs e)
    {
        bool swFuncionario = true;
        for (int i = 0; i < gv_planilla.Rows.Count; i++)
        {
            GridViewRow row = gv_planilla.Rows[i];
            bool isChecked = ((CheckBox)row.FindControl("chk_marcar")).Checked;
        ...
        if (swFuncionario)
        {
            sc = "MostrarMascara(false); $('#modificarFecha').modal('show');";
            SetScript(sc);
        }
        else
        {
            sc = "MostrarMascara(false); $.notify({ icon: 'fa fa-info', message:'Seleccione a los funcionarios a modificar.'},{type: 'warning', placement: { from: 'bottom', align: 'right'} });";
            SetScript(sc);
        }
    }

    protected void btn_confirmar_cambio_Click(object sender, EventArgs e)
    {
        DateTime pre_fecha_inicio;
        DateTime? pre_fecha_inicio_aux = null;
        DateTime? pre_fecha_fin_aux = null;
        for (int i = 0; i < gv_planilla.Rows.Count; i++)
        {
            GridViewRow row = gv_planilla.Rows[i];
            bool isChecked = ((CheckBox)row.FindControl("chk_marcar")).Checked;
            ...
            if (isChecked)
            {
                pre_fecha_inicio_aux = (DateTime)Convert.ToDateTime(gv_planilla.Rows[i].Cells[10].Text);
        ...
        for (int i = 0; i < gv_planilla.Rows.Count; i++)
        {
            ...
            if (isChecked)
            {
                pre_fecha_inicio = (DateTime)Convert.ToDateTime(gv_planilla.Rows[i].Cells[10].Text);
                if (pre_fecha_inicio_aux != pre_fecha_inicio)
                {
                    sw = false;
                    break;
                }
        ...
        if (sw)
        {
            DateTime pre_fecha_inicio_nuevo = Convert.ToDateTime(txt_nueva_fecha_inicio.Text);
            ...
            for (int i = 0; i < gv_planilla.Rows.Count; i++)
            {
                ...
                {
                    pre_fecha_inicio_aux = (DateTime)Convert.ToDateTime(gv_planilla.Rows[i].Cells[10].Text);
                    pre_fecha_fin_aux = (DateTime)Convert.ToDateTime(gv_planilla.Rows[i].Cells[11].Text);

                    if (pre_fecha_inicio_nuevo >= pre_fecha_inicio_aux && pre_fecha_inicio_nuevo <= pre_fecha_fin_aux)
                    {
                        sw_aux = true;
                    } else
                    {
            ...
            if (sw_aux)
            {
                for (int i = 0; i < gv_planilla.Rows.Count; i++)
                {
                    ...
                    if (isChecked)
                    {
                        int pre_id = (int)Convert.ToInt32(gv_planilla.DataKeys[i].Values[0].ToString());
                        precontratado = new cls_pc_precontratado();
                        precontratado.pre_id = pre_id;
                        precontratado.pre_fecha_inicio = txt_nueva_fecha_inicio.Text;
                        precontratado.ActualizarFecha();

                    }
                }
                txt_nueva_fecha_inicio.Text = "";
                int pl_id = (Request.QueryString["id"] != null) ? Convert.ToInt32(Request.QueryString["id"].ToString()) : 0;
                listarPlanilla(pl_id);
                sc = "MostrarMascara(false); Swal.fire({ icon: 'success', title: 'Fechas modificadas con éxito.', text: 'Registro exitoso', timer: 2200, showConfirmButton: false, allowOutsideClick: false, onAfterClose: () => { $('#modificarFecha').modal('hide'); $('body').removeClass('modal-open'); $('.modal-backdrop').remove(); }});";
                SetScript(sc);
            } else
            {
                sc = "MostrarMascara(false); $.notify({ icon: 'fa fa-info', message:'Los precontratos seleccionados están fuera del rango de fecha permitidos.'},{type: 'warning', placement: { from: 'bottom', align: 'right'} });";
                SetScript(sc);
            }

        } else
        {
            sc = "MostrarMascara(false); $.notify({ icon: 'fa fa-info', message:'Los precontratos seleccionados deben fechas ser iguales.'},{type: 'warning', placement: { from: 'bottom', align: 'right'} });";
            SetScript(sc);
        }
    }
}