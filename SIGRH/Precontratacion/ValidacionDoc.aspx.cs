...
public partial class Precontratacion_ValidacionDoc : System.Web.UI.Page
{
            ...
            foreach (GridViewRow row in gv_planilla.Rows)
            {
                for (int i = 0; i < gv_planilla.Columns.Count; i++)
                {
                    if (i == 15)
                    {
                        ...
                        if (estado == "OBSERVADO")
                        {
                            row.CssClass = "grid-row-warning2";
                        }
                        if (estado == "VALIDADO POR RRHH")
                        {
            ...
            case "GetAssign":
                if (pre_estado != null && pre_estado != "")
                {
                    if (pre_estado.Trim() != "O")
                    {
                        ...
                        datosFuncionario();
                        contruirDocumentosVacio(requisitos);
            ...
            if (requisitos.Tables[0].Rows.Count > 0)
            {
                ...
                for (int i = 0; i < nro_requisito; i++)
                {
                    ...
                    if (lista_respuesta.Rows.Count == 2)
                    {
                        ...
                        lit.Text = "<div id='d_" + validarCampo(requisito_x["rq_id"]) + "' class='form-group row' style='margin-bottom: unset;' >";
                        ...
                        {
                            if (validarCampo(lista_respuesta.Rows[j]["rc_desc"]) == "SI")
                            {
                                HiddenField hf_si = new HiddenField();
                                hf_si.ID = "hf_rq_rc_si_" + validarCampo(requisito_x["rq_id"]);
                                hf_si.Value = validarCampo(lista_respuesta.Rows[j]["rc_id"]);
                                pnlDinamico.Controls.Add(hf_si);
                            }
                            if (validarCampo(lista_respuesta.Rows[j]["rc_desc"]) == "NO")
                            {
                                HiddenField hf_no = new HiddenField();
                                hf_no.ID = "hf_rq_rc_no_" + validarCampo(requisito_x["rq_id"]);
                                hf_no.Value = validarCampo(lista_respuesta.Rows[j]["rc_id"]);
                                pnlDinamico.Controls.Add(hf_no);
                            }
                        }

                        //col3
                        Literal litCk = new Literal();
                        litCk.Text = "<div class='col-md-12'> <div class='custom-control custom-checkbox mb-lst-chk'>";
                        pnlDinamico.Controls.Add(litCk);

                        CheckBox chk = new CheckBox();
                        chk.ID = "chk_rq_" + validarCampo(requisito_x["rq_id"]);
                        chk.InputAttributes["class"] = "custom-control-input";
                        pnlDinamico.Controls.Add(chk);

                        Label ckLabel = new Label();
                        ckLabel.ID = "lbl_ck_id" + validarCampo(requisito_x["rq_id"]);
                        ckLabel.Text = validarCampo(requisito_x["rq_descripcion"]);
                        ckLabel.CssClass = "custom-control-label ";
                        ckLabel.AssociatedControlID = "chk_rq_" + validarCampo(requisito_x["rq_id"]);
                        pnlDinamico.Controls.Add(ckLabel);

                        Literal litCk2 = new Literal();
                        litCk2.Text = "</div></div>";
                        pnlDinamico.Controls.Add(litCk2);
    ...
    protected void btn_guardar_requisitos_Click(object sender, EventArgs e)
    {
        ...
        if (requisitos.Tables[0].Rows.Count > 0)
        {
            ...
            {
                string id_chk = "chk_rq_" + validarCampo(requisitos.Tables[0].Rows[i]["rq_id"]);
                CheckBox chk = (CheckBox)pnlDinamico.FindControl(id_chk);

                string id_h1 = "hf_rq_rc_si_" + validarCampo(requisitos.Tables[0].Rows[i]["rq_id"]);
                HiddenField hf_si = (HiddenField)pnlDinamico.FindControl(id_h1);

                string id_h2 = "hf_rq_rc_no_" + validarCampo(requisitos.Tables[0].Rows[i]["rq_id"]);
                HiddenField hf_no = (HiddenField)pnlDinamico.FindControl(id_h2);

                if (hf_si != null && hf_no != null)
                {
                    dr = lista_respuesta.NewRow();
                    dr["rp_rq_id"] = Convert.ToInt32(validarCampo(requisitos.Tables[0].Rows[i]["rq_id"]));
                    dr["rp_rc_id"] = (chk.Checked) ? Convert.ToInt32(hf_si.Value) : Convert.ToInt32(hf_no.Value);
                    lista_respuesta.Rows.Add(dr);
                }
            }
            ...
            resp_combo.rq_categoria = "PRECONTRATOS";
            resp_combo.rp_nombre_pk = "pre_id";
            resp_combo.rp_valor_pk = Convert.ToInt32(hf_pre_id.Value);
            var detalle_requisitos_presentados = resp_combo.RequisitosPresentadosFun();
            precontratado = new cls_pc_precontratado { pre_id = Convert.ToInt32(hf_pre_id.Value) };
            var detalle_precontrato = precontratado.DatosPrecontratoUDEP();

            if (detalle_requisitos_presentados.Tables[0].Rows.Count > 0 && detalle_precontrato.Tables[0].Rows.Count > 0)
            {
                bool sw = true;
                var requisitos_presentados = detalle_requisitos_presentados.Tables[0].Rows;
                var precontrato = detalle_precontrato.Tables[0].Rows[0];
                string documentosFaltantes = "";
                for (int i = 0; i < requisitos_presentados.Count; i++)
                {
                    switch (validarCampo(requisitos_presentados[i]["rp_rq_id"]))
                    {
                        case "26":
                            if (validarCampo(precontrato["es_descripcion"]).Contains("PROFESIONAL"))
                            {
                                sw = (validarCampo(requisitos_presentados[i]["rc_desc"]) == "SI");
                            }
                            break;
                        case "27":
                            if (validarCampo(precontrato["es_descripcion"]).Contains("PROFESIONAL"))
                            ...
                            break;
                        case "28":
                            if (validarCampo(precontrato["genero_x"]) == "M")
                            {
                                sw = (validarCampo(requisitos_presentados[i]["rc_desc"]) == "SI");
                            }
                            break;
                        ...
                        case "31":
                            ...
                            break;
                        case "32":
                            if (validarCampo(precontrato["pre_presenta_djbr"]) == "SI")
                            {
                                sw = (validarCampo(requisitos_presentados[i]["rc_desc"]) == "SI");
                            }
                            break;
                        case "38":
                            if (validarCampo(precontrato["estado_fun"]) == "NUEVO")
                            {
                                sw = (validarCampo(requisitos_presentados[i]["rc_desc"]) == "SI");
                            }
                            break;
                ...
                for (int i = 0; i < requisitos_presentados.Count; i++)
                {
                    switch (validarCampo(requisitos_presentados[i]["rp_rq_id"]))
                    {
                        case "26":
                            if (validarCampo(precontrato["es_descripcion"]).Contains("PROFESIONAL"))
                            {
                                documentosFaltantes = (validarCampo(requisitos_presentados[i]["rc_desc"]) != "SI") ? documentosFaltantes + validarCampo(requisitos_presentados[i]["rq_descripcion"]) + "</br>" : documentosFaltantes + "";
                            }
                            break;
                        case "27":
                            if (validarCampo(precontrato["es_descripcion"]).Contains("PROFESIONAL"))
                ...
                {
                    precontratado.pre_id = Convert.ToInt32(hf_pre_id.Value);
                    precontratado.pre_estado = "VR";
                    precontratado.PrecontratoEstadoUDEP();
                }
                else
                {
                    precontratado.pre_id = Convert.ToInt32(hf_pre_id.Value);
                    precontratado.pre_estado = "V";
                    precontratado.PrecontratoEstadoUDEP();
                }
                //actualizarEstadoPlanilla();
                informacionPlanilla(pl_id);
                ...
                {
                    alert = "Faltan documentos para su validación: ', html: '" + documentosFaltantes;
                }
                else
                {
                    alert = "No se presentó ningún documento.";
                }
                sc = (sw) ? "Swal.fire({ icon: 'success', title: 'Documentos validados exitosamente.', text: 'Registro exitoso', timer: 2200, showConfirmButton: false, allowOutsideClick: false, onAfterClose: () => { $('#modalVerificarDoc').modal('hide'); $('body').removeClass('modal-open'); $('.modal-backdrop').remove(); }});" : " Swal.fire({icon: 'info', title: '" + alert + "', showConfirmButton: true, allowOutsideClick: false, onAfterClose: () => { $('#modalVerificarDoc').modal('hide'); $('body').removeClass('modal-open'); $('.modal-backdrop').remove(); }});";
                SetScript(sc);
            }
            else
            {
    ...
    private void contruirDocumentosVacio(DataSet requisitos = null)
    {
        int nro_requisito = 0;
        if (requisitos.Tables.Count > 0)
        {
            if (requisitos.Tables[0].Rows.Count > 0)
            {
                ...
                for (int i = 0; i < nro_requisito; i++)
                {
                    ...
                    string id_chk = "chk_rq_" + validarCampo(requisito_x["rq_id"]);
                    CheckBox chk = (CheckBox)pnlDinamico.FindControl(id_chk);
                    if (chk != null)
                    {
                        chk.Checked = false;
    ...
    private void documentosFiltrados(DataSet requisitos = null)
    {
        precontratado = new cls_pc_precontratado { pre_id = Convert.ToInt32(hf_pre_id.Value) };
        var precontrato = precontratado.DatosPrecontratoUDEP();

        if (requisitos.Tables[0].Rows.Count > 0 && precontrato.Tables[0].Rows.Count > 0)
        {
            var detalle_requisitos = requisitos.Tables[0].Rows;
            var detalle_precontrato = precontrato.Tables[0].Rows[0];

            DataTable lista_ocultar = new DataTable();
            lista_ocultar.Columns.Add("rq_id");
            DataRow dr = null;

            string sc_aux = "";
            for (int i = 0; i < detalle_requisitos.Count; i++)
            {
                string d_id = "d_" + validarCampo(detalle_requisitos[i]["rq_id"]);
                sc_aux = sc_aux + "$('#" + d_id + "').css('display', 'block'); ";
                bool sw = true;
                switch (validarCampo(detalle_requisitos[i]["rq_id"]))
                {
                    case "26":
                        sw = (validarCampo(detalle_precontrato["es_descripcion"]).Contains("PROFESIONAL"));
                        break;
                    case "27":
                        sw = (validarCampo(detalle_precontrato["es_descripcion"]).Contains("PROFESIONAL"));
                        break;
                    case "28":
                        sw = (validarCampo(detalle_precontrato["genero_x"]) == "M");
                        break;
                    case "29":
                        sw = (validarCampo(detalle_precontrato["genero_x"]) == "M");
                        break;
                    case "31":
                        ...
                        break;
                    case "32":
                        sw = (validarCampo(detalle_precontrato["pre_presenta_djbr"]) == "SI");
                        break;
                    case "38":
                        sw = (validarCampo(detalle_precontrato["estado_fun"]) == "NUEVO");
                        break;
                ...
                if (!sw)
                {
                    dr = lista_ocultar.NewRow();
                    dr["rq_id"] = Convert.ToInt32(validarCampo(detalle_requisitos[i]["rq_id"]));
                    lista_ocultar.Rows.Add(dr);
                }
            }
            SetScript(sc_aux);

            sc_aux = "";
            for (int i = 0; i < lista_ocultar.Rows.Count; i++)
            {
                for (int j = 0; j < detalle_requisitos.Count; j++)
                {
                    string id_chk = "chk_rq_" + validarCampo(detalle_requisitos[j]["rq_id"]);
                    CheckBox chk = (CheckBox)pnlDinamico.FindControl(id_chk);

                    string id_lbl = "lbl_ck_id" + validarCampo(detalle_requisitos[j]["rq_id"]);
                    Label lbl = (Label)pnlDinamico.FindControl(id_lbl);

                    string d_id = "d_" + validarCampo(detalle_requisitos[j]["rq_id"]);

                    if (validarCampo(detalle_requisitos[j]["rq_id"]) == validarCampo(lista_ocultar.Rows[i]["rq_id"]))
                    {
                        if (chk != null && lbl != null)
                        {
                            sc_aux = sc_aux + "$('#" + d_id + "').css('display', 'none'); ";
                        }
                        break;
                    } 
                }
            ...
            SetScript(sc_aux);
            //string json = JsonConvert.SerializeObject(lista_ocultar);
        }
        else
        {
            sc = "$.notify({ icon: 'fa fa-exclamation', message: 'Ocurrió un error'},{type: 'warning', placement: { from: 'bottom', align: 'right'} }); ";
    ...
    private void requisitosPresentados(DataSet requisitos = null)
    {
        ...
        if (requisitos_presentados.Tables[0].Rows.Count > 0)
        {
            for (int i = 0; i < requisitos_presentados.Tables[0].Rows.Count; i++)
            {
                ...
                string id_h = "hf_" + i;
                HiddenField hf = (HiddenField)pnlDinamico.FindControl(id_h);

                if (hf != null)
                {
                    if (hf.Value == validarCampo(funcionario["rp_rq_id"]))
                    {
                        string id_chk = "chk_rq_" + validarCampo(funcionario["rp_rq_id"]);
                        CheckBox chk = (CheckBox)pnlDinamico.FindControl(id_chk);
                        chk.Checked = (validarCampo(funcionario["rc_desc"]) == "SI");
                    }
                }
            }
        }
        ...
        {
            marcarPorDefecto(requisitos);
        }
    }
    private void marcarPorDefecto(DataSet lista_requisitos = null)
    {
        int[] rq_id = { 20, 21, 22, 23, 25, 34 };

        if (lista_requisitos.Tables.Count > 0)
        {
            if (lista_requisitos.Tables[0].Rows.Count > 0)
            {
                var requisitos = lista_requisitos.Tables[0].Rows;
                for (int i = 0; i < rq_id.Length; i++)
                {
                    for (int j = 0; j < requisitos.Count; j++)
                    {
                        if (rq_id[i] == Convert.ToInt32(validarCampo(requisitos[j]["rq_id"])))
                        {
                            string id_chk = "chk_rq_" + validarCampo(requisitos[j]["rq_id"]);
                            CheckBox chk = (CheckBox)pnlDinamico.FindControl(id_chk);
                            if (chk != null)
                            {
                                chk.Checked = true;
    ...
    protected void btn_adicionar_glosa_Click(object sender, EventArgs e)
    {
        ...
        if (hf_val_rrhh.Value == "1")
        {
            ...
            planilla.seg_pk_id = pl_id;
            planilla.seg_us_id_remitente = Convert.ToInt32(Session["us_id"]);
            planilla.seg_us_id_recepcion = 0;
            planilla.seg_accion = "VALIDADO POR RRHH";
