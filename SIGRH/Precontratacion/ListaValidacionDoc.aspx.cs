...
public partial class Precontratacion_ListaValidacionDoc : System.Web.UI.Page
{
    ...
    private void listarMes()
    {
        ...
        catalogo.cat_tabla = "mes";

        DataView dv = new DataView();
        dv.Table = catalogo.ObtenerTablaCombo().Tables[0];
        dv.Sort = "cat_secuencial";

        ddl_mes.Items.Insert(0, new ListItem("Seleccione...", "0"));
        ddl_mes.DataSource = dv;
    ...
    private void listarEstados()
    {
        ...
        itemToRemove = ddl_estado.Items.FindByValue("C");
        if (itemToRemove != null)
        {
            ddl_estado.Items.Remove(itemToRemove);
        }

        itemToRemove = null;
        itemToRemove = ddl_estado.Items.FindByValue("E");
        if (itemToRemove != null)
        {
            ddl_estado.Items.Remove(itemToRemove);
        }
        ...
        itemToRemove = null;
        itemToRemove = ddl_estado.Items.FindByValue("S");
        if (itemToRemove != null)
        {
            ddl_estado.Items.Remove(itemToRemove);
        }
        ...
        itemToRemove = null;
        itemToRemove = ddl_estado.Items.FindByValue("P");
        if (itemToRemove != null)
        {
            ddl_estado.Items.Remove(itemToRemove);
        }
        ...
        itemToRemove = null;
        itemToRemove = ddl_estado.Items.FindByValue("A");
        if (itemToRemove != null)
        {
            ddl_estado.Items.Remove(itemToRemove);
        }
    ...
    private void listarPlanillas(int us_id = 0, int pr_id = 0)
    {
        ...
        planilla = new cls_pc_precontratado { usuario_per_id = us_id, pl_pr_id = Convert.ToString(pr_id), pl_mes = Convert.ToInt32(mes) };
    ...
    protected void lv_planilla_ItemCommand(object sender, ListViewCommandEventArgs e)
    {
        ...
        {
            case "GetAssign":
                if (us_id == 414 || us_id == 417 || us_id == 1007)
                {
                    Response.Redirect("ValidacionDocDesc?id=" + pl_id + "&id2=" + pl_ue + "&id3=" + Session["pr_id"].ToString());
                }
                else
                {
                    Response.Redirect("ValidacionDoc?id=" + pl_id + "&id2=" + pl_ue + "&id3=" + Session["pr_id"].ToString());
                }
    ...
    protected void btn_buscar_planilla_Click(object sender, EventArgs e)
    {
        ...
        planilla = new cls_pc_precontratado
        {
            ...
            pl_mes = Convert.ToInt32(ddl_mes.SelectedValue)
        };
