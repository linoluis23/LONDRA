...
public partial class Precontratacion_PlanillaAprobacion : System.Web.UI.Page
{
    ...
    private void verificarValidacion()
    {
        ...
        switch (ltl_pl_estado.Text)
        {
            case "ENVIADO":
                ...
                //    btn_aprobar.Text = "<span class='btn-inner--icon'><i class='fas fa-check'></i></span><span class='btn-inner--text'>APROBADO</span>";
            ...
            case "APROBADO":
                ...
                break;
            case "VALIDADO":
                btn_aprobar.CssClass = "btn btn-vimeo btn-round btn-icon disabled";
    ...
    protected void btn_confirmar_ajuste_Click(object sender, EventArgs e)
    {
        ...
        if (tam_cat > 0)
        {
            if (validarCampo(estados.Tables[0].Rows[0]["seg_accion"]) == "ENVIADO")
            {
                precontratado = new cls_pc_precontratado { pre_pl_id = pl_id, pre_estado = "E", pre_estado_x = "A", pl_estado = "E", pl_estado_x = "A", pl_observaciones = (txt_observaciones.Text.Trim() != "") ? txt_observaciones.Text.ToUpper().Trim() : null };
                ...
                string correo_recepcion = validarCampo(estados.Tables[0].Rows[0]["correo_remitente"]);
                string usuario_recepcion = validarCampo(estados.Tables[0].Rows[0]["remitente"]);
                planilla.seg_pk_id = pl_id;
                planilla.seg_us_id_remitente = us_id;
                planilla.seg_us_id_recepcion = Convert.ToInt32(validarCampo(estados.Tables[0].Rows[0]["seg_us_id_remitente"]));
    ...
    protected void btn_confirmar_envio_Click(object sender, EventArgs e)
    {
        ...
        if (tam_cat > 0)
        {
            if (validarCampo(estados.Tables[0].Rows[0]["seg_accion"]) == "ENVIADO")
            {
                ...
                planilla.seg_accion = "APROBADO";
                planilla.seg_observaciones = null;
                planilla.seg_tabla = "tbl_pc_planilla";
                planilla.AdicionarSeguimiento();

                string asunto = "SOLICITUD DE REVISIÓN Y VALIDACIÓN DE LA PLANILLA Nº " + ltl_pl_correlativo.Text;
                string contenido = "<p>Se le comunica que se aprobó la <strong>Planilla Nº " + ltl_pl_correlativo.Text + "</strong>, por el usuario <strong>" + usuario_remitente + "</strong> y se envía para para su revisión y validación.</p>";
                enviarCorreo(correo_remitente, correo_recepcion, usuario_remitente, usuario_recepcion, contenido, asunto);
