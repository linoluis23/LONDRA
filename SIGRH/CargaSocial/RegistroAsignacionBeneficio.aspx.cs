...
public partial class BienestarSocial_RegistroAsignacionBeneficio : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!Page.IsPostBack)
        {
            ...
            cargarDatosFiliacion(as_id, codFun);
            Session["pf_id"] = null;
            Session["pers_id"] = codFun;
    ...
    private void cargarDatosFiliacion(int as_id, int per_id)
    {
    ...
    protected void gv_familiares_RowCommand(object sender, GridViewCommandEventArgs e)
    {
        ...
        switch (e.CommandName)
        {
            ...
            case "GetAfili":
                Session["pf_id"] = pf_id;
                sc = "$('#RegistrarAfiliacionBeneficiario').modal('show');";
                SetScript(sc);
                break;
            default:
                break;
    ...
    protected void llenarDatosFamiliar(int pf_id = 0)
    {
        ...
        txt_ci_edit.Text = validarCampo(familiarX["ci"]);
    ...
    protected void seleccionarBeneficiario(int pf_id = 0)
    {
        ...
        var detalleFamiliar = beneficio.ObtenerFamiliar();
        var familiarX = detalleFamiliar.Tables[0].Rows[0];
        matricula = validarCampo(familiarX["nro_matricula"]);
        if (matricula != "No tiene matricula")
        {
            txt_matri_benef.Text = validarCampo(familiarX["nro_matricula"]);
            txt_matri_benef.Enabled = false;
        }
        ...
        {
            if (ddl_tipo_beneficio.SelectedValue == "1018" || ddl_tipo_beneficio.SelectedValue == "1019")
            {
    ...
    {
        benef.ap_esposo = txt_ap_esposo_fam.Text;
        benef.ap_materno = txt_ap_materno_fam.Text;
        benef.ap_paterno = txt_ap_paterno_fam.Text;
        benef.fecha_nac = txt_fecha_nac_fam.Text;
        benef.pf_ci = txt_ci_edit.Text;
        benef.pf_id = Convert.ToInt32(aux_pf_id.Value);
        benef.pf_nombre = txt_nombres_fam.Text;
        benef.tipo_parentesco = Convert.ToInt32(ddl_pf_tipo_parentesco.SelectedValue);
        benef.pf_sexo = ddl_tipo_genero.SelectedValue.ToString();
        benef.EditarDatosFamiliar();
        ...
        //familiar.pf_ci = txt_ci_edit.Text;
        //familiar.ActualizarDatosFamiliarNuevo();
    }
    ...
    protected string calcularTiempoMeses()
    {
        ...
        beneficio = new cls_bs_asignacion_beneficio
        {
            ...
            ab_fecha_fin = Session["fecha_fin"].ToString()
        };
    ...
    {
        int meses = 0;
        if (ddl_tipo_beneficio.SelectedValue == "1018")
        {
            meses = 4;
        }
        else if (ddl_tipo_beneficio.SelectedValue == "1019")
        {
            meses = 12;
        }
        DateTime today = Convert.ToDateTime(txt_a_partir.Text);
        DateTime firstDayOfMonth = new DateTime(today.Year, today.Month, today.Day);
        DateTime lastDayOfMonth = firstDayOfMonth.AddMonths(meses);
        string hasta = lastDayOfMonth.ToString("dd/MM/yyyy");
        Session["fecha_fin"] = hasta;
        beneficio = new cls_bs_asignacion_beneficio();
        int per_id = (Request.QueryString["id"] != null) ? Convert.ToInt32(Request.QueryString["id"].ToString()) : 0;
        Session["pers_id"] = per_id;
        int pf_id = (aux_pf_id.Value != "") ? Convert.ToInt32(aux_pf_id.Value) : 0;
        string fecha_inicio = txt_a_partir.Text;
        string fecha_fin = hasta;
        DateTime startDate = Convert.ToDateTime(txt_a_partir.Text);
        DateTime endDate = Convert.ToDateTime(hasta);
        ...
        if (diferencia > 0)
        {
            switch (ddl_tipo_beneficio.SelectedValue)
            {
                ...
                case "1018":
                    if (pf_id == 0)
                    {
                        var verificaSubsidioMes = beneficio.VerificarSubsidioenMes(Convert.ToString(per_id), ddl_tipo_beneficio.SelectedValue, fecha_inicio, fecha_fin);
                        if (verificaSubsidioMes.Tables[0].Rows.Count > 0)
                        {
                            Limpiar();
                            sc = "$.notify({ icon: 'fa fa-check', message: 'No se puede realizar la acción, se realizó un registro anteriormente.'},{type: 'info', placement: { from: 'bottom', align: 'right'} });$('#modalGlosa').modal('hide'); $('body').removeClass('modal-open'); $('.modal-backdrop').remove();$('#block_mesAsignado').css('display', 'none');$('#block_mesAsignado').css('display', 'none');$('#block_gvFamiliares').css('display', 'none');";
    ...
    private void listarGrillaAsignacionBeneficio()
    {
        ...
        try
        {
            ...
            {
                beneficiarios = benef.ObtenerDatosBeneficiario(per_id);
                if (beneficiarios.Tables[0].Rows.Count > 0)
                {
                    gv_asignacion_beneficios.DataSource = beneficiarios;
                    this.block_familia.Visible = false;
    ...
    {
        if (Session["pf_id"] == null)
        {
            int x = Convert.ToInt32(Session["pers_id"]);
    ...
    {
        string algo = Session["fecha_fin"].ToString();
        beneficio = new cls_bs_asignacion_beneficio
        {
            ab_aeb_id = Convert.ToInt32(aux_ab_aeb_id.Value),
            ab_fa_id = Convert.ToInt32(ddl_tipo_beneficio.SelectedValue),
    ...
    protected void Limpiar()
    {
        ...
        txt_matri_benef.Text = string.Empty;
    }
    ...
    private void GuardarNuevoFamiliar()
    {
        ...
        familiar = new cls_bs_asignacion_beneficio
        {
            ...
            pf_ci = txt_ci_famili.Text
        };
        familiar.AdicionarFamiliarNuevo();
    }
    ...
    protected void btn_cancelarSubsidio_Click(object sender, EventArgs e)
    {
        ...
        if (aux_accion.Value == "1")
        {
            ...
            sc = "Swal.fire({ icon: 'success', title: 'Activación exitosa', text: 'Subsidio activado correctamente', timer: 2200, showConfirmButton: false, allowOutsideClick: true, onAfterClose: () => { $('#cancelarSubsidio').modal('hide'); $('body').removeClass('modal-open'); $('.modal-backdrop').remove(); }});";
        }
        else
        {
            beneficio.ab_estado = "C";
            beneficio.CancelarSubsidio();
            sc = "Swal.fire({ icon: 'success', title: 'Cancelación exitosa', text: 'Subsidio cancelado correctamente', timer: 2200, showConfirmButton: false, allowOutsideClick: true, onAfterClose: () => { $('#cancelarSubsidio').modal('hide'); $('body').removeClass('modal-open'); $('.modal-backdrop').remove(); }});";
        }
        listarGrillaAsignacionBeneficio();
        SetScript(sc);
    }
    ...
    protected void btn_adicionar_glosa_Click(object sender, EventArgs e)
    {
        ...
        gv_familiares.DataSource = null;
        gv_familiares.DataBind();
        sc = "MostrarMascara(false); Swal.fire({ icon: 'success', title: 'Asignación exitosa', text: 'Registro exitoso', timer: 2200, showConfirmButton: false, allowOutsideClick: true, onAfterClose: () => { $('#block_mesAsignado').css('display', 'none');$('#block_mesAsignado').css('display', 'none');$('#block_gvFamiliares').css('display', 'none');$('#modalGlosa').modal('hide'); $('body').removeClass('modal-open'); $('.modal-backdrop').remove(); }});";
        SetScript(sc);
    }
    ...
    private void GuardarAfiliacionFamiliares(int pf_id = 0)
    {
        familiar = new cls_bs_asignacion_beneficio
        {
            ...
            pf_id = pf_id,
            aeb_afi_por = "3",
            nro_matri = txt_nro_matricula_benefic.Text
        };
        familiar.AdicionarAfiliacionFamiliarNuevo();
    }

    protected void btn_autoasig_Click(object sender, EventArgs e)
    {
        Subsidio benef = new Subsidio();
        string matricula = "";
        int per_id = (Request.QueryString["id"] != null) ? Convert.ToInt32(Request.QueryString["id"].ToString()) : 0;
        var detalleFamiliar = benef.ObtenerDatosPersona(per_id);
        var familiarX = detalleFamiliar.Tables[0].Rows[0];
        matricula = validarCampo(familiarX["matricula"]);
        
        txt_matri_benef.Text = matricula;
        txt_matri_benef.Enabled = false;
        
        txt_nombre_beneficiarioS.Text = validarCampo(familiarX["per_ap_paterno"]) + " " + validarCampo(familiarX["per_ap_materno"]) + " " + validarCampo(familiarX["per_nombres"]);
        txt_nombre_beneficiarioS.Enabled = false;
        if (ddl_tipo_beneficio.SelectedValue == "1018" || ddl_tipo_beneficio.SelectedValue == "1019")
        {
            sc = "$('#block_datosBeneficiario').css('display', 'block');$('#block_mesAsignado').css('display', 'block'); $('#block_asigFamiliares').css('display', 'block');";
    ...
    protected void gv_familiares_RowDataBound(object sender, GridViewRowEventArgs e)
    {
        if (e.Row.RowType != DataControlRowType.DataRow) { return; }
        Subsidio benef = new Subsidio();
        int num = 0;
        var index = Convert.ToInt32(e.Row.DataItemIndex);
        var id = gv_familiares.DataKeys[index].Value.ToString();
        familiar = new cls_bs_asignacion_beneficio();
        familiar.pf_per_id = Convert.ToInt32(Session["pers_id"]);
        var var_datos_as_c = familiar.ListarFamiliares().Tables[0];

        if (var_datos_as_c.Rows.Count > 0)
        {
            num = benef.VerificarAfiliacionFamiliar(Convert.ToInt32(id));
            if (num > 0 )
            {
                LinkButton lnkBtnB = (LinkButton)e.Row.FindControl("btn_asignar");
                lnkBtnB.Visible = true;
                LinkButton lnkBbtnAfili = (LinkButton)e.Row.FindControl("btn_Afiliar");
                lnkBbtnAfili.Visible = false;
            }
            else
            {
                LinkButton lnkBtnB = (LinkButton)e.Row.FindControl("btn_asignar");
    ...
    protected void btn_afiliar_benef_Click(object sender, EventArgs e)
    {
        GuardarAfiliacionFamiliares(Convert.ToInt32(Session["pf_id"]));
        familiar = new cls_bs_asignacion_beneficio();
        familiar.pf_per_id = Convert.ToInt32(Session["pers_id"]);
        var familiaresX = familiar.ListarFamiliares();
        gv_familiares.DataKeyNames = new string[] { "pf_id" };
        gv_familiares.DataSource = familiaresX;
        gv_familiares.DataBind();
        sc = "Swal.fire({ icon: 'success', title: 'Activación exitosa', text: 'Afiliacion Correcta', timer: 2200, showConfirmButton: false, allowOutsideClick: true, onAfterClose: () => { $('#RegistrarAfiliacionBeneficiario').modal('hide'); $('body').removeClass('modal-open'); $('.modal-backdrop').remove(); $('#block_gvFamiliares').css('display', 'none'); }});";
        SetScript(sc);
    }
}