...
public partial class MasterPageSIGRH : System.Web.UI.MasterPage
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (HttpContext.Current.Session["per_id"] != null)
        {
            ...
            {
                ltl_nombre_fun.Text = Session["per_nombres"].ToString() + ": " + HttpContext.Current.Session["rol"].ToString();
        ...
        //Mostrando Notificaciones
        if (HttpContext.Current.Session["texto_notificacion_error"] != null)
        {
            if (HttpContext.Current.Session["texto_notificacion_error"].ToString() != "")
            {
                string texto_notificacion = HttpContext.Current.Session["texto_notificacion_error"].ToString();
                ScriptManager.RegisterStartupScript(this.Page, this.GetType(), "notificacion", "$.notify({ icon: 'fa fa-times-circle', message: '" + texto_notificacion + "'},{ type: 'danger', placement: { from: 'bottom', align: 'right'} });", true);
                ScriptManager.RegisterStartupScript(this.Page, this.GetType(), "notificacion", "Swal.fire({ icon: 'success', title: '" + texto_notificacion + "', text: 'Registro exitoso', timer: 2200, showConfirmButton: false, allowOutsideClick: false});", true);
                Session["texto_notificacion_error"] = "";
            }
        }
    }
