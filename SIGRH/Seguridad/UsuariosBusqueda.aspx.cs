...
public partial class Seguridad_UsuariosBusqueda : System.Web.UI.Page
{
    ...
    private void BindGridView(string varId, string varCed, string varPat, string varMat, string varNom, string varCas)
    {
        try
        {
            ...
            GvLista.DataSource = _persona.ObtenerTablaGrilla__gamlp_vigente(varId, "", varCed, "", varPat, varMat, varNom, varCas, "", "", "", "", "", "", Convert.ToInt32(Session["pr_id"].ToString())); // _persona.ObtenerTablaGrilla(varId, "", varCed, "", varPat, varMat, varNom, varCas, "", "", "", "", "", "");
            GvLista.DataBind();
        }
    ...
    protected void GvLista_RowCommand(object sender, GridViewCommandEventArgs e)
    {
        ...
        var dataU = _usuario.ObtenerTablaGrilla("", "", "", code, "", "", "", "", "", "", "", "", "V").Tables[0];
        ...
        if (dataU.Rows.Count > 0) sc = "$.notify({ icon: 'fas fa-exclamation', message: 'El funcionario ya tiene asignado un usuario vigente...!!' }, { type: 'warning' });";
        else { if (e.CommandName.Equals("GetNew")) Response.Redirect("UsuariosAlta?id=" + code); }

        //if (data.Rows.Count > 0) { if (e.CommandName.Equals("GetNew")) Response.Redirect("SancionesAlta?id=" + code); }
