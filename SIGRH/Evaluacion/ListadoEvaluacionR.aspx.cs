...
public partial class Evaluacion_ListadoEvaluacionR : System.Web.UI.Page
{
    ...
    private void DatosPersonales_Bind()
    {
        if (periodo == 1)
        {
            Labeleva.Text = "Primer Semestre " + cls_Formulario.GestionActual("gestion", tipo).Tables[0].Rows[0]["periodo_gestion"].ToString();
        }
    ...
    {
        cls_Formulario.Finalizar_Evaluacion(Convert.ToInt32(lblEvaluado.Text), "finalizado");
        Gestion_Bind(tipo);
        DatosPersonales_Bind();
        Dependientes_Bind();
        string sc;
