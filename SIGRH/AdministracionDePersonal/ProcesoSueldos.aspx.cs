...
public partial class AdministracionDePersonal_ProcesoSueldos : System.Web.UI.Page
{
    ...
    private void inforamcionSalarioMinimo()
    {
        ...
        if (detalleFuncionario.Tables.Count > 0)
        {
            if (detalleFuncionario.Tables[0].Rows.Count > 0)
            {
                ...
                if (cargo.ConsultaSiPlanillaEstaEjecutada(txt_pc_id.Text).Tables[0].Rows[0][0].ToString() == "0")
                {
                    lblMes.Text = "Proceso Actual: " + validarCampo(funcionario["pc_titulo"]);
    ...
    {   //Proceso inicial
        ...
        proceso1.EjecutarProceso1(Convert.ToInt32(txt_pc_id.Text));
    ...
    {   //Cotizables
        ...
        proceso2.EjecutarProceso2(Convert.ToInt32(txt_pc_id.Text));
        ...
        DataSet dss = proceso2.VerificarCasosDoblePercepcion(Convert.ToInt32(txt_pc_id.Text), "C1");
        if (dss.Tables[0].Rows.Count > 0)
        {
            GvLista.DataSource = proceso.VerificarCasosDoblePercepcion(Convert.ToInt32(txt_pc_id.Text), "C1");
            GvLista.DataBind();
            divProceso3_1.Visible = true;
            btnProceso3.Enabled = false;
        ...
        else
        {
            ...
            //divProceso4.Visible = true;
            divProcesoSanciones.Visible = true;
            btnProceso3.Enabled = false;
            DivPanelActualizacion.Visible = false;
        }
    ...
    {   //Afp
        ...
        proceso3.EjecutarProceso3(Convert.ToInt32(txt_pc_id.Text));
    ...
    {
        cls_pla_proceso_salarios proceso3 = new cls_pla_proceso_salarios();
        proceso3.EjecutarProceso4(Convert.ToInt32(txt_pc_id.Text));
    ...
    {
        cls_pla_proceso_salarios proceso = new cls_pla_proceso_salarios();
        return proceso.AjusteDoblePercepcion(ti_tipo, cbh_id, horas, ganado, esc_por, esc, bono_a, bono_f);
    }
    protected void GvLista_RowCommand(object sender, GridViewCommandEventArgs e)
    {
        ...
        if (e.CommandName.Equals("AjustarDPercepcion"))
        {
            DivDoblePercepcion.Visible = true;
            cls_pla_proceso_salarios proceso = new cls_pla_proceso_salarios();
            gvDoblePercepcion.DataSource = proceso.VerificarCasosDoblePercepcion(Convert.ToInt32(per_id), "C2");
    ...
    private void VerificarDoblePercepcion_()
    {
        ...
        if (dss.Tables[0].Rows.Count > 0)
        {
            GvLista.DataSource = proceso2.VerificarCasosDoblePercepcion(Convert.ToInt32(txt_pc_id.Text), "C1");
            GvLista.DataBind();
            divProceso3_1.Visible = true;
            btnProceso3.Enabled = false;
            divProceso4.Visible = false;
        ...
        else
        {
            ...
            btDPfake.Enabled = false;
            divProcesoSanciones.Visible = true;
            btnProcesoSanciones.Visible = true;
            divProceso4.Visible = false;
            btnProceso3.Enabled = false;
            DivPanelActualizacion.Visible = false;
        }
    ...
    {
        cls_pla_proceso_salarios proceso = new cls_pla_proceso_salarios();
        if (proceso.AplicarAjusteDoblePercepcion(hdf_ti_tipo.Value, Convert.ToInt32(hdf_cbh_id.Value), Convert.ToInt32(hdf_horas.Value), Convert.ToDouble(hdf_ganado.Value), Convert.ToDouble(hdf_esc_por.Value), Convert.ToDouble(hdf_esc.Value), Convert.ToDouble(hdf_bono_a.Value), Convert.ToDouble(hdf_bono_f.Value)))
    ...
    protected void btnProcesoSanciones_Click(object sender, EventArgs e)
    {
        cls_pla_proceso_salarios proceso = new cls_pla_proceso_salarios();
        proceso.Procesar_Sanciones(Convert.ToInt32(hf_pc_id.Value));
        btnProcesoSanciones.Enabled = false;
        divProceso4.Visible = true;
        DivPanelActualizacion.Visible = false;
