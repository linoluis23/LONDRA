...
public partial class Configuraciones_RegistroAdminFactores : System.Web.UI.Page
{
    ...
    protected void gv_factor_PreRender(object sender, EventArgs e)
    {
        base.OnPreRender(e);
        if (gv_factor.Rows.Count > 0)
        {
            if (gv_factor.HeaderRow != null)
            {
                gv_factor.HeaderRow.TableSection = TableRowSection.TableHeader;
            }
            if (gv_factor.FooterRow != null)
            {
                gv_factor.FooterRow.TableSection = TableRowSection.TableFooter;
            }
        }
    }

    protected void gv_factor_RowCommand(object sender, GridViewCommandEventArgs e)
    {
        int index = Convert.ToInt32(e.CommandArgument);
        string fa_id = gv_factor.DataKeys[index].Values[0].ToString();
    ...
    private void ListarFactores()
    {
        try
        {
            ...
            gv_factor.DataSource = factor.ObtenerGrillaFactores();
            gv_factor.DataBind();
        }
        catch (Exception e)
        {
    ...
    protected void btn_nuevo_factor_Click(object sender, EventArgs e)
    {
        Limpiar();
        aux_accion.Value = "0";
        ltl_titulo.Text = "REGISTRAR FACTOR";
        sc = "$('#modalNuevoFactor').modal('show');";
        SetScript(sc);
    }
