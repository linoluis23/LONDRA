...
using System.Text;
public partial class MovimientoPersonal_ListaPlanillaContrato : System.Web.UI.Page
{
            ...
            case "GetAssign":
                ...
                switch (pl_estado)
                {
                    case "VALIDADO POR RRHH":
                        Response.Redirect("PlanillaContrato?id=" + pl_id + "&id2=" + pl_ue + "&id3=" + Session["pr_id"].ToString());
                        break;
                    case "VALIDADO":
                        sc = "$.notify({ icon: 'fa fa-info-circle', message: 'No se puede realizar la acción, la planilla aún no ha sido validada por Recursos Humanos.'},{type: 'info', placement: { from: 'bottom', align: 'right'} });";
                        SetScript(sc);
                        break;
                    case "EJECUTADO":
                        //sc = "$.notify({ icon: 'fa fa-info-circle', message: 'No se puede realizar la acción, los contratos de la planilla ya fueron ejecutados.'},{type: 'info', placement: { from: 'bottom', align: 'right'} });";
                        //SetScript(sc);
                        Response.Redirect("PlanillaContrato?id=" + pl_id + "&id2=" + pl_ue + "&id3=" + Session["pr_id"].ToString());
                        break;
                    default:
    ...
    protected void btn_buscar_planilla_Click(object sender, EventArgs e)
    {
        ...
        if (tam_cat > 0)
        {
            ...
            SetScript("$('#d_planillas').css('display', 'block');");
        }
        else
        {
