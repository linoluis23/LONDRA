...
public partial class MovimientoPersonal_Lista_Item : System.Web.UI.Page
{
    ...
    private void listarItems()
    {
        try
        {
            ...
            gv_items.DataSource = cargo.ObtenerGrillaItemConsejo();
            gv_items.DataBind();
        }
        catch (Exception e)
        {
    ...
    protected void gv_items_RowCommand(object sender, GridViewCommandEventArgs e)
    {
        ...
        asignacion.as_per_id = Convert.ToInt32(per_id);
        var detalleAsignacion = asignacion.ObtenerDetalleValidacion();
        string as_estado = "";
        as_estado = (detalleAsignacion.Tables[0].Rows.Count > 0) ? validarCampo(detalleAsignacion.Tables[0].Rows[0]["as_validacion"]) : "";
        ...
        switch (e.CommandName)
        {
            case "GetAssign":
                if (as_estado == "S")
                {
                    Response.Redirect("AsignacionFuncionarioConsejo?id=" + per_id);
                } else
                {
                    sc = "$.notify({ icon: 'fas fa-exclamation', message: 'No se puede realizar la acción, la asignación del funcionario no fue validada.' }, { type: 'warning', placement: { from: 'bottom', align: 'right'} }); $('#blockResultados').css('display', 'none');";
                    SetScript(sc);
                }
    ...
    private void BindGridView(string varId, string varCed, string varPat, string varMat, string varNom, string varCas)
    {
        try
        {
            ...
            gv_items.DataSource = _persona.ObtenerTablaGrilla__persona_concejo(varId, "", varCed, "", varPat, varMat, varNom, varCas, "", "", "", "", "", "", Convert.ToInt32(gestion) );
            gv_items.DataBind();
        }
