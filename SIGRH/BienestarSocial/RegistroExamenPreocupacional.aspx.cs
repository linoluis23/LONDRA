...
public partial class BienestarSocial_RegistroExamenPreocupacional : System.Web.UI.Page
{
    ...
    protected void gv_items_RowCommand(object sender, GridViewCommandEventArgs e)
    {
        ...
        string as_id = gv_items.DataKeys[index].Values[1].ToString();
        string exp_id = gv_items.DataKeys[index].Values[2].ToString();

        string nua = gv_items.Rows[index].Cells[7].Text;
        string fecha_examen = gv_items.Rows[index].Cells[8].Text;
        string fecha_dec_examen = gv_items.Rows[index].Cells[9].Text;
        ...
        switch (e.CommandName)
        {
            case "GetAssign":
                //if (nua != "")
                //{
                //    Response.Redirect("ExamenPreocupacional?id=" + per_id + "&id2=" + as_id);
                //}
                //else
                //{
                //    sc = "$.notify({ icon: 'fa fa-exclamation', message: 'No se puede realizar la acción, el funcionario no tiene AFP registrada.'},{type: 'warning', placement: { from: 'bottom', align: 'right'} }); ";
                //    SetScript(sc);
                //}
                Response.Redirect("ExamenPreocupacional?id=" + per_id + "&id2=" + as_id);
                break;
            case "GetAssignConfirm":
                Response.Redirect("DeclararExamenPreocupacional?id=" + per_id + "&id2=" + as_id + "&id3=" + exp_id);
                break;
            case "GetAssignAnalisis":
                Response.Redirect("AnalisisExPreo?id=" + per_id + "&id2=" + as_id + "&id3=" + exp_id);
                break;
            default:
                break;
        }
    ...
    private void BindGridView(string varId, string varCed, string varPat, string varMat, string varNom, string varCas)
    {
        ...
        try
        {
            ...
            gv_items.DataSource = _persona.ObtenerTablaGrilla__gamlp_vigente_ex_preocupacional(varId, "", varCed, "", varPat, varMat, varNom, varCas, "", "", "", "", "", "", pr_id);
            gv_items.DataBind();
        }
