...
public partial class BienestarSocial_BuscadorFuncionarioAfiliacionEGS : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!Page.IsPostBack)
        {
            listaFiltradoCajaAseguradora();
            listaFiltradoTipoDoc();
            listaFiltradoEGS();
            listaFiltradoEstadoAfiliacion();
        }
    }
    ...
    protected void gv_items_RowCommand(object sender, GridViewCommandEventArgs e)
    {
        ...
        //aux_ti_item.Value = gv_items.DataKeys[index].Values[4].ToString();
        string fa_id = gv_items.DataKeys[index].Values[3].ToString();
        string per_id = aux_per_id.Value;
        string as_id = aux_as_id.Value;
        //string ti_item = aux_ti_item.Value;
        switch (e.CommandName)
        {
            ...
            case "GetEdit":
                //if (ti_item == "P")
                //{
                //    int idAsignacion = obtenerIdAsignacion(Convert.ToInt32(per_id));
                //    Response.Redirect("RegistroAfiliacionEGS?id=" + per_id + "&id2=" + idAsignacion + "&id3=" + fa_id);
                //}
                //else
                //{
                //    Response.Redirect("RegistroAfiliacionEGS?id=" + per_id + "&id2=" + as_id + "&id3=" + fa_id);
                //}
                Response.Redirect("RegistroAfiliacionEGS?id=" + per_id + "&id2=" + as_id + "&id3=" + fa_id);
                break;
            default:
    ...
    private int obtenerIdAsignacion(int per_id = 0)
    {
        beneficio = new cls_bs_asignacion_beneficio();
        beneficio.pf_per_id = per_id;
        var ae_as_id = beneficio.obtenerIdAsignacionEGS();
        int idAsignacion = 0;
        if (ae_as_id.Tables[0].Rows[0]["ae_as_id"] != DBNull.Value && ae_as_id.Tables[0].Rows[0]["ae_as_id"].ToString().Trim() != "")
        {
            idAsignacion = Convert.ToInt32(ae_as_id.Tables[0].Rows[0]["ae_as_id"]);
        }
        return idAsignacion;
    }
    ...
    {
        if (string.IsNullOrEmpty(Txt_per_num_doc_b.Text) && string.IsNullOrEmpty(Txt_per_ap_paterno_b.Text) && string.IsNullOrEmpty(Txt_per_ap_materno_b.Text) && string.IsNullOrEmpty(Txt_per_nombres_b.Text)  && string.IsNullOrEmpty(Txt_per_id_b.Text) && (ddl_tipo_egs.SelectedValue != null && ddl_tipo_egs.SelectedValue != "0") && (ddl_estado_afiliacion.SelectedValue != null && ddl_estado_afiliacion.SelectedValue != "0" && ddl_estado_afiliacion.SelectedValue == ""))
            sc = "$.notify({ icon: 'fas fa-exclamation', message: 'Debe ingresar algún parámetro de búsqueda...!!' }, { type: 'warning' });";
        else
        {
            BindGridView(Txt_per_id_b.Text.Trim(), Txt_per_num_doc_b.Text.Trim(), Txt_per_ap_paterno_b.Text.Trim(), Txt_per_ap_materno_b.Text.Trim(), Txt_per_nombres_b.Text.Trim(), ddl_tipo_egs.SelectedValue.Trim(), ddl_estado_afiliacion.SelectedValue.Trim());
            Limpiar("sch_cl");
    ...
    private void BindGridView(string varId, string varCed, string varPat, string varMat, string varNom, string varEgs, string varAfi)
    {
        try
        {
            _persona = new cls_persona();
            gv_items.DataSource = _persona.ObtenerTablaGrilla__personaGamlp_AfiliacionEGS(varId, "", varCed, "", varPat, varMat, varNom, "", "", "", "", "", "", "", varEgs, varAfi);
            gv_items.DataBind();
        }
    ...
    private void Limpiar(string val)
    {
        ...
        if (val.Equals("sch_cl"))
        {
            ...
            ddl_caja_aseguradora.SelectedValue = "221";
            ddl_tipo_afiliación.SelectedValue = "2";
    ...
    private void listaFiltradoCajaAseguradora()
    {
        try
        {
            beneficio = new cls_bs_asignacion_beneficio();
    ...
    private void listaFiltradoEGS()
    {
        try
        {
            beneficio = new cls_bs_asignacion_beneficio();

            ddl_tipo_egs.Items.Clear();
            ddl_tipo_egs.Items.Insert(0, new ListItem("Seleccione...", "0"));
            ddl_tipo_egs.DataValueField = "fa_id";
            ddl_tipo_egs.DataTextField = "fa_descripcion";
            ddl_tipo_egs.DataSource = beneficio.listaFiltradoCajaAseguradora();
            ddl_tipo_egs.DataBind();
    ...
    private void listaFiltradoEstadoAfiliacion()
    {
        try
        {
            beneficio = new cls_bs_asignacion_beneficio();

            ddl_estado_afiliacion.Items.Clear();
            ddl_estado_afiliacion.Items.Insert(0, new ListItem("Seleccione...", "0"));
            ddl_estado_afiliacion.DataValueField = "ae_estado";
            ddl_estado_afiliacion.DataTextField = "ae_estado";
            ddl_estado_afiliacion.DataSource = beneficio.listaFiltradoEstadoAfiliacion();
            ddl_estado_afiliacion.DataBind();
    ...
    {
        //ddl_caja_aseguradora.SelectedValue = "37";
        restablecerGlosa();
    ...
    {
        ddl_tipo_egs.SelectedValue = "0";
        ddl_estado_afiliacion.SelectedValue = "0";
        Limpiar("gv_cl");
        sc = "$('#blockResultados').css('display', 'none');";
        SetScript(sc);
    }
    ...
    protected void btn_adicionar_glosa_Click(object sender, EventArgs e)
    {
        ...
        if (detalle_afiliacion.Tables[0].Rows.Count > 0)
        {
            ...
            {
                Session["texto_notificacion"] = "¡Afiliación exitosa!";
                Response.Redirect("RegistroAfiliacionEGS?id=" + per_id_aux + "&id2=" + as_id_aux + "&id3=" + fa_id_aux);
            }
            else
            {
        ...
        Limpiar("gv_cl");
        //llenarDatosAfiliacionX(per_id);
    }

    protected void btn_cancelar_glosa_Click1(object sender, EventArgs e)
    {
